...
{
    public class STMCAN2 : IDoubleWordPeripheral, ICAN, INumberedGPIOOutput
    {
        public STMCAN2(STMCAN2 master = null)
        {
            // Register master CAN
            this.master = master;
        ...
        public void WriteDoubleWord(long address, uint value)  // cpu do per
        {
            ...
            switch((registerOffset)address)
            {
                case registerOffset.CAN_MSR:
                    this.Log(LogLevel.Warning, "Writing CAN_MSR, value={0:X}", value);
                    registers.CAN_MSR.SetValue(value);
                    UpdateSCEInterruptLine();
                    return;
                case registerOffset.CAN_MCR:
                    this.Log(LogLevel.Warning, "Writing CAN_MCR, value={0:X}", value);
                    registers.CAN_MCR.SetValue(value);

                    if(registers.CAN_MCR.InitRequest == true &&
                       registers.CAN_MCR.SleepRequest == true)                          //INFO changed from false to true, now it works...
                    {
                    ...
                    else
                    {
                        ...
                        registers.CAN_MSR.SleepAck = false;
                        registers.CAN_MSR.InitAck = false;
                        this.Log(LogLevel.Warning, "Else-Block, CAN_MCR.InitRequest={0:X}, CAN_MCR.SleepRequest={0:X}", registers.CAN_MCR.InitRequest, registers.CAN_MCR.SleepRequest);
                    }
            ...
            else
            {
                switch((registerOffset)offset)
                {
                    case registerOffset.CAN_MCR:
                        ...
                        this.Log(LogLevel.Warning, "Reading CAN_MCR, value={0:X}", Retval);
                        break;
                    case registerOffset.CAN_MSR:
                        Retval = registers.CAN_MSR.GetValue();
                        this.Log(LogLevel.Warning, "Reading CAN_MSR, value={0:X}", Retval);
                        break;
                    case registerOffset.CAN_TSR:
        ...
        private readonly STMCAN2 master;
